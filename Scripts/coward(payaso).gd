...
# Flee
@export var flee_radius: float = 100.0          # distancia para empezar a huir
@export var return_to_wander_radius: float = 140.0  # distancia para volver a wander (> flee_radius)
...
func update_state():
	...
	if current_state == State.WANDER and dist < flee_radius:
		current_state = State.FLEE
	elif current_state == State.FLEE and dist > return_to_wander_radius:
